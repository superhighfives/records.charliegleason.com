 ...
 * edge-snap because the paid Magic matte failed (`professionalAlphaSource === "deterministic"`).
 * Informational, not a failure — the cover is fine — so it reads amber and subtle. Lets you
 * spot which records could be upgraded (via "Retry Magic matte") without opening each one.
 */
export function MatteFallbackBadge({ className }: { className?: string }) {
	return (
		<span
			...
			title="Cover is live, but the matte used the deterministic fallback — the Magic matte can be retried"
		>
			Lo-fi matte
		</span>
	);
