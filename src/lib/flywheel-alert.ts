 ...
 * retraining until enough corrections have accrued, and no automated retrain (a model swap is
 * reviewed, not silent). So this weekly cron simply *counts* how many labels have changed
 * since the model was last trained, and emails once the drift crosses {@link RETRAIN_THRESHOLD}
 * so the admin can run the ~3-min flywheel (see `ml/README.md`) when convenient.
 *
...
async function countDrift(): Promise<{ added: number; changed: number }> {
...
export function runFlywheelCheck(): Promise<{
	...
	return Sentry.startSpan({ name: "runFlywheelCheck" }, async () => {
		...
		if (drift < RETRAIN_THRESHOLD || !env.EMAIL) {
			return { added, changed, emailed: false };
		}
		await env.EMAIL.send({
			from: FROM,
			to: TO,
			subject: `${drift} sleeve crops changed — retrain the corner detector?`,
			html:
		...
		return { added, changed, emailed: true };
	});
}
